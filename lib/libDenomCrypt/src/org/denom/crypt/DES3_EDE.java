...
/**
 * Шифр TripleDES (DES-EDE3).
 */
public class DES3_EDE extends ABlockCipher
{
	...
	// -----------------------------------------------------------------------------------------------------------------
	/**
	 * @param key Ключ [24 байта]
	 */
	public DES3_EDE( final Binary key )
	{
		des1 = new DES( new Binary( BLOCK_SIZE ) );
		des2 = new DES( new Binary( BLOCK_SIZE ) );
		des3 = new DES( new Binary( BLOCK_SIZE ) );
		setKey( key );
	}

	// -----------------------------------------------------------------------------------------------------------------
	@Override
	public DES3_EDE clone()
	{
		return new DES3_EDE( this.getKey() );
	}
	 ...
	 * @param key Ключ [24 байт]
	 */
	@Override
	public DES3_EDE setKey( final Binary key )
	{
		MUST( key.size() == KEY_SIZE, "Wrong key size" );
		des1.setKey( key.first( DES.KEY_SIZE ) );
		des2.setKey( key.slice( DES.KEY_SIZE, DES.KEY_SIZE ) );
		des3.setKey( key.last( DES.KEY_SIZE ) );
		return this;
	}
	...
	{
		return Bin( des1.getKey(), des2.getKey(), des3.getKey() );
	}
	...
	public void calcCCSlast( final Binary data, Binary ccs )
	{
		...
		switch( ccs_mode )
		{
			...
			case FAST:
				...
				des3.encryptFirst( Bin(), Bin(), CryptoMode.ECB, AlignMode.NONE, ccsFast_IV );
				des3.encryptLast( ccs, ccs );
				break;
		}
	}
}

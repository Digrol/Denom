...
import org.denom.crypt.ec.rfc7748.Ed448;

import static org.denom.Binary.Bin;
import static org.denom.Ex.MUST;

public class TestEd448
{
	private final SecureRandom RANDOM = new SecureRandom();

	public TestEd448( ILog log )
	{
		log.write( getClass().getSimpleName() + "... " );

		Ed448.precompute();

		testEd448Consistency();
		...
		checkEd448Vector(
			...
			"03",
			"666f6f",
			"d4f8f6131770dd46f40867d6fd5d5055de43541f8c5e35abbcd001b32a89f7d2151f7647f11d8ca2ae279fb842d607217fce6e042f6815ea00"
	...
	// -----------------------------------------------------------------------------------------------------------------
	private void testEd448Consistency()
	{
		...
		for( int i = 0; i < 10; ++i )
		{
			...
			MUST( Arrays.equals( sig1, sig2 ), "Consistent signatures #" + i );
			MUST( Ed448.verify( sig1, 0, pk, 0, ctx, m, 0, mLen ), "Consistent sign/verify #" + i );

			sig1[ Ed448.PUBLIC_KEY_SIZE - 1 ] ^= 0x80;
			MUST( !Ed448.verify( sig1, 0, pk, 0, ctx, m, 0, mLen ), "Consistent verification failure #" + i );
		}
	}

	// -----------------------------------------------------------------------------------------------------------------
	private void testEd448Vector4()
	{
		checkEd448Vector(
...
//	@Test
	private void testEd448Vector64()
	{
		String m =
...
//	@Test
	private void testEd448Vector256()
	{
		String m =
...
//	@Test
	private void testEd448Vector1023()
	{
		String m =
	...
	{
		byte[] sk = Bin( sSK ).getBytes();
		byte[] pk = Bin( sPK ).getBytes();
		byte[] pkGen = new byte[ Ed448.PUBLIC_KEY_SIZE ];
		Ed448.generatePublicKey( sk, 0, pkGen, 0 );
		MUST( Arrays.equals( pk, pkGen ), text );

		byte[] m = Bin( sM ).getBytes();
		byte[] ctx = Bin( sCTX ).getBytes();
		byte[] sig = Bin( sSig ).getBytes();
		byte[] sigGen = new byte[ Ed448.SIGNATURE_SIZE ];
		Ed448.sign( sk, 0, ctx, m, 0, m.length, sigGen, 0 );
		MUST( Arrays.equals( sig, sigGen ), text );

		Ed448.sign( sk, 0, pk, 0, ctx, m, 0, m.length, sigGen, 0 );
		MUST( Arrays.equals( sig, sigGen ), text );
		MUST( Ed448.verify( sig, 0, pk, 0, ctx, m, 0, m.length ), text );

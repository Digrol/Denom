...
class TestHC
{
	...
	// -----------------------------------------------------------------------------------------------------------------
	private void testHC( StreamCipher hc, String test, byte[] key, byte[] IV, byte[] expected )
	{
		...
		for( int i = 0; i < 64; i++ )
		{
			MUST( hc.returnByte( MSG[ i ] ) == expected[ i ] );
		}
	}
	...
	TestHC()
	{
		StreamCipher hc = new HC256();

		for( int i = 0; i != HC256_VerifiedTest.length; i++ )
		{
			String[] test = HC256_VerifiedTest[ i ];
			testHC( hc, "HC-256 - " + test[ 0 ], HexDecode( test[ 1 ] ), HexDecode( test[ 2 ] ), HexDecode( test[ 3 ] ) );
		}
		...
		for( int i = 0; i != HC128_VerifiedTest.length; i++ )
		{
			...
			testHC( hc, "HC-128 - " + test[ 0 ], HexDecode( test[ 1 ] ), HexDecode( test[ 2 ] ), HexDecode( test[ 3 ] ) );
		}

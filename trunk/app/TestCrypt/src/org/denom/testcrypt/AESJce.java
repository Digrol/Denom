...
/**
 * AES от стандартного крипто-провайдера.
 */
public class AESJce
{
	...
	// -----------------------------------------------------------------------------------------------------------------
	public AESJce( final Binary key )
	{
		setKey( key );
	}
	...
	public Binary encrypt( final Binary data, CryptoMode crypt_mode, AlignMode align_mode, final Binary iv )
	{
		try
		{
			...
			SecretKeySpec skey = new SecretKeySpec( key.getDataRef(), 0, key.size(), "AES" );
	...
	public Binary decrypt( final Binary crypt, CryptoMode crypt_mode, AlignMode align_mode, final Binary iv )
	{
		try
		{
			...
			Binary data = new Binary( cipher.doFinal( crypt.getBytes() ) );

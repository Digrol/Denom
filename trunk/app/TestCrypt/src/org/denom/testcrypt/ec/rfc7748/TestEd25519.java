...
import org.denom.crypt.ec.rfc7748.Ed25519;
...
public class TestEd25519
{
	...
	{
		new TestEd25519( new LogConsole() );
	}

	// -----------------------------------------------------------------------------------------------------------------
	public TestEd25519( ILog log )
	{
		log.write( getClass().getSimpleName() + "... " );

		Ed25519.precompute();

		testEd25519Consistency();
	...
	// -----------------------------------------------------------------------------------------------------------------
	private void testEd25519Consistency()
	{
		...
		for( int i = 0; i < 10; ++i )
		{
			...
			MUST( shouldVerify, "Consistent sign/verify #" + i );

			sig1[ Ed25519.PUBLIC_KEY_SIZE - 1 ] ^= 0x80;
			boolean shouldNotVerify = Ed25519.verify( sig1, 0, pk, 0, m, 0, mLen );
			MUST( !shouldNotVerify, "Consistent verification failure #" + i );
		}
	}

	// -----------------------------------------------------------------------------------------------------------------
	private void testEd25519Vector1023()
	{
		String m =
	...
	private void checkEd25519Vector( String sSK, String sPK, String sM, String sSig, String text )
	{
		...
		MUST( Arrays.equals( sig, sigGen ), text );
		MUST( Ed25519.verify( sig, 0, pk, 0, m, 0, m.length ), text );

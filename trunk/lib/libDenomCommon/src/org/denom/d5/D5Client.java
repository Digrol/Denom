...
public class D5Client implements AutoCloseable
{
	...
	public void command( final D5Command cmd, D5Response response )
	{
		...
		{
			log.writeln( String.format( "D5Command : %08X %08X %08X %s",
					cmd.index, cmd.code, cmd.data.size(), cmd.data.Hex() ) );
		}
		...
		if( (log != null) && !(log instanceof LogDummy) )
		{
			log.writeln( String.format( "D5Response: %08X %08X %08X %08X %s",
					response.index, response.code, response.status, response.data.size(), response.data.Hex() ) );
		}
	 ...
	 * @return reference on D5Response, kept in object. NOT COPY.
	 */

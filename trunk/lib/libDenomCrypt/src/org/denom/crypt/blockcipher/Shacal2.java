 ...
 * http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.3.4066 Best known cryptanalytic
 * (Wikipedia 11.2013): Related-key rectangle attack on 44-rounds (Jiqiang Lu, Jongsung Kim).
 * Comments are related to SHA-256-Naming as described in FIPS PUB 180-2
 * </p>
 */
public class Shacal2 implements BlockCipher
{
	...
	private void encryptBlock( byte[] in, int inOffset, byte[] out, int outOffset )
	{
		...
		{
			int tmp = (((block[ 4 ] >>> 6) | (block[ 4 ] << -6)) ^ ((block[ 4 ] >>> 11) | (block[ 4 ] << -11)) ^ ((block[ 4 ] >>> 25) | (block[ 4 ] << -25))) + ((block[ 4 ] & block[ 5 ]) ^ ((~block[ 4 ]) & block[ 6 ])) + block[ 7 ] + K[ i ] + workingKey[ i ]; // corresponds to T1 of FIPS PUB 180-2
			block[ 7 ] = block[ 6 ];
			block[ 6 ] = block[ 5 ];
			block[ 5 ] = block[ 4 ];
			block[ 4 ] = block[ 3 ] + tmp;
	...
	private void decryptBlock( byte[] in, int inOffset, byte[] out, int outOffset )
	{
		...
		{
			int tmp = block[ 0 ] - (((block[ 1 ] >>> 2) | (block[ 1 ] << -2)) ^ ((block[ 1 ] >>> 13) | (block[ 1 ] << -13)) ^ ((block[ 1 ] >>> 22) | (block[ 1 ] << -22))) - ((block[ 1 ] & block[ 2 ]) ^ (block[ 1 ] & block[ 3 ]) ^ (block[ 2 ] & block[ 3 ])); // T2
			block[ 0 ] = block[ 1 ];
			block[ 1 ] = block[ 2 ];
			block[ 2 ] = block[ 3 ];
			block[ 3 ] = block[ 4 ] - tmp;
			...
			block[ 5 ] = block[ 6 ];
			block[ 6 ] = block[ 7 ];
			block[ 7 ] = tmp - K[ i ] - workingKey[ i ] - (((block[ 4 ] >>> 6) | (block[ 4 ] << -6)) ^ ((block[ 4 ] >>> 11) | (block[ 4 ] << -11)) ^ ((block[ 4 ] >>> 25) | (block[ 4 ] << -25))) - ((block[ 4 ] & block[ 5 ]) ^ ((~block[ 4 ]) & block[ 6 ])); // T1
		}
		ints2bytes( block, out, outOffset );
	}
	...
	public int processBlock( byte[] in, int inOffset, byte[] out, int outOffset )
	{
		...
		MUST( (inOffset + BLOCK_SIZE) <= in.length );
		MUST( (outOffset + BLOCK_SIZE) <= out.length );

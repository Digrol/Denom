 ...
 * the NIST AES Quest.
 * <p>
 * For full details see <a href="http://www.cl.cam.ac.uk/~rja14/serpent.html">The Serpent home
 * page</a>
 */
public final class Serpent extends SerpentBase
{
	...
	protected int[] makeWorkingKey( byte[] key ) throws IllegalArgumentException
	{
		...
		{
			kPad[ length++ ] = Static.littleEndianToInt( key, off );
		}
	...
	protected void encryptBlock(
		...
		int	 outOff)
	{
		X0 = Static.littleEndianToInt(input, inOff);
		X1 = Static.littleEndianToInt(input, inOff + 4);
		X2 = Static.littleEndianToInt(input, inOff + 8);
		X3 = Static.littleEndianToInt(input, inOff + 12);
		...
		Static.intToLittleEndian(wKey[128] ^ X0, output, outOff);
		Static.intToLittleEndian(wKey[129] ^ X1, output, outOff + 4);
		Static.intToLittleEndian(wKey[130] ^ X2, output, outOff + 8);
		Static.intToLittleEndian(wKey[131] ^ X3, output, outOff + 12);
	}
	...
	protected void decryptBlock(
		...
		int	 outOff)
	{
		X0 = wKey[128] ^ Static.littleEndianToInt(input, inOff);
		X1 = wKey[129] ^ Static.littleEndianToInt(input, inOff + 4);
		X2 = wKey[130] ^ Static.littleEndianToInt(input, inOff + 8);
		X3 = wKey[131] ^ Static.littleEndianToInt(input, inOff + 12);
		...
		Static.intToLittleEndian(X0 ^ wKey[0], output, outOff);
		Static.intToLittleEndian(X1 ^ wKey[1], output, outOff + 4);
		Static.intToLittleEndian(X2 ^ wKey[2], output, outOff + 8);
		Static.intToLittleEndian(X3 ^ wKey[3], output, outOff + 12);
	}
}

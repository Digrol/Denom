 ...
 */
public class Threefish implements BlockCipher
{
	 ...
	 *
	 * @param blocksizeBits the block size in bits, one of {@link #BLOCKSIZE_256},
	 * {@link #BLOCKSIZE_512}, {@link #BLOCKSIZE_1024}.
	 */
	public Threefish( final int blocksizeBits )
	{
		...
		switch( blocksizeBits )
		{
			...
			case BLOCKSIZE_1024:
				...
				break;
			default:
				THROW( "Invalid blocksize - Threefish is defined with block size of 256, 512, or 1024 bits" );
		}
	}

	// -----------------------------------------------------------------------------------------------------------------
	public static class TweakableParams implements CipherParameters
	{
		final byte[] tweak;
		final KeyParameter keyParams;

		public TweakableParams( final KeyParameter key, final byte[] tweak )
		{
			this.keyParams = key;
	 ...
	 *
	 * @param params an instance of {@link TweakableParams}, or {@link KeyParameter}
	 * (to use a 0 tweak)
	 */
	public void init( boolean forEncryption, CipherParameters params )
	{
		...
		if( params instanceof TweakableParams )
		{
			TweakableParams tParams = (TweakableParams)params;
			keyBytes = tParams.keyParams.getKey();
	...
	private void setKey( long[] key )
	{
		 ...
		 * 20k for 1024). Key and tweak word sequences are repeated, and static MOD17/MOD9/MOD5/MOD3
		 * calculations used, to avoid expensive mod computations during cipher operation.
		 */
	...
	{
		MUST( (inOff + blocksizeBytes) <= in.length );
		MUST( (outOff + blocksizeBytes) <= out.length );
	...
	{
		MUST( kw[ blocksizeWords ] != 0 );

		if( forEncryption )
		{
			cipher.encryptBlock( currentBlock, currentBlock );
		}
		else
		{
			cipher.decryptBlock( currentBlock, currentBlock );
		}
	...
	private static final class Threefish256Cipher extends ThreefishCipher
	{
		...
		void encryptBlock( long[] block, long[] out )
		{
			...
			for( int d = 1; d < (ROUNDS_256 / 4); d += 2 )
			{
				...
				/*
				 * 4 rounds of mix and permute. Permute schedule has a 2 round cycle, so permutes

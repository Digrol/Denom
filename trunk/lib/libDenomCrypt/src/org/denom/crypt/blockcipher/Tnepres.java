 ...
 */
public final class Tnepres extends SerpentBase
{
	...
	protected int[] makeWorkingKey( byte[] key ) throws IllegalArgumentException
	{
		...
		{
			kPad[ length++ ] = Static.bigEndianToInt( key, off );
		}

		if( off == 0 )
		{
			kPad[ length++ ] = Static.bigEndianToInt( key, 0 );
			if( length < 8 )
			{
				kPad[ length ] = 1;
			}
	...
	protected void encryptBlock(
		...
		int	 outOff)
	{
		X3 = Static.bigEndianToInt(input, inOff);
		X2 = Static.bigEndianToInt(input, inOff + 4);
		X1 = Static.bigEndianToInt(input, inOff + 8);
		X0 = Static.bigEndianToInt(input, inOff + 12);
		...
		Static.intToBigEndian(wKey[131] ^ X3, output, outOff);
		Static.intToBigEndian(wKey[130] ^ X2, output, outOff + 4);
		Static.intToBigEndian(wKey[129] ^ X1, output, outOff + 8);
		Static.intToBigEndian(wKey[128] ^ X0, output, outOff + 12);
	}
	...
	protected void decryptBlock(
		...
		int	 outOff)
	{
		X3 = wKey[131] ^ Static.bigEndianToInt(input, inOff);
		X2 = wKey[130] ^ Static.bigEndianToInt(input, inOff + 4);
		X1 = wKey[129] ^ Static.bigEndianToInt(input, inOff + 8);
		X0 = wKey[128] ^ Static.bigEndianToInt(input, inOff + 12);
		...
		Static.intToBigEndian(X3 ^ wKey[3], output, outOff);
		Static.intToBigEndian(X2 ^ wKey[2], output, outOff + 4);
		Static.intToBigEndian(X1 ^ wKey[1], output, outOff + 8);
		Static.intToBigEndian(X0 ^ wKey[0], output, outOff + 12);
	}
}

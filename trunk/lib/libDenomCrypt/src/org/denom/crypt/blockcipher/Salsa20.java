...
public class Salsa20 implements StreamCipher
{
	...
	private final static int[] TAU_SIGMA = Static.littleEndianToInt( "expand 16-byte kexpand 32-byte k".getBytes(), 0, 8 );
	...
	public int processBytes( byte[] in, int inOff, int len, byte[] out, int outOff )
	{
		...
		MUST( !limitExceeded( len ), "2^70 byte limit per IV would be exceeded; Change IV" );
	...
	protected void setKey( byte[] keyBytes, byte[] ivBytes )
	{
		if( keyBytes != null )
		{
			...
			// Key
			Static.littleEndianToInt( keyBytes, 0, engineState, 1, 4 );
			Static.littleEndianToInt( keyBytes, keyBytes.length - 16, engineState, 11, 4 );
		}

		// IV
		Static.littleEndianToInt( ivBytes, 0, engineState, 6, 2 );
	}

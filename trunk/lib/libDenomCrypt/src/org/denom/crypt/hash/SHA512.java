 ...
 */
public class SHA512 extends IHash
{
	...
	protected void finish()
	{
		...
		tail.resize( BLOCK_SIZE );
		tail.setLong( tail.size() - 16, processedBytes >>> 61 );
	...
	protected void processBlock( Binary data, int offset )
	{
		...
		for( int j = 0; j < 16; ++j, offset += 8 )
		{
			W[ j ] = data.getLong( offset );
			...
			long ch = (e & f) ^ (~e & g);
			long t1 = h + s1 + ch + CONSTANTS[ j ] + W[ j ];
		...
		for( int j = 16; j < 80; ++j )
		{
			long s0 = rotateRight( W[j - 15], 1 ) ^ rotateRight( W[j - 15], 8 ) ^ (W[j - 15] >>> 7);
			long s1 = rotateRight( W[j - 2], 19 ) ^ rotateRight( W[j - 2], 61 ) ^ (W[j - 2] >>> 6);

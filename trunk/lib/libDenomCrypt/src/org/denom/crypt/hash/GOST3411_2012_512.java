 ...
 */
public class GOST3411_2012_512 extends IHash
{
	...
	{
		reversedBlock.assign( data, offset, BLOCK_SIZE );
		reversedBlock.reverse();

		G( N, reversedBlock.getDataRef() );
		add( N, 512 );
		add( Sigma, reversedBlock.getDataRef() );
	}
	...
	private void finish()
	{
		...
		tail.set( tailLen, 0x01 );
		tail.reverse();

		G( N, tail.getDataRef() );
		add( N, tailLen << 3 );
		add( Sigma, tail.getDataRef() );
	...
	// -----------------------------------------------------------------------------------------------------------------
	private void F( byte[] arr )
	{
		for( int i = 0; i < 8; ++i )
		{
			long l = T0[ arr[56 + i] & 0xFF ];
			l ^= T1[ arr[48 + i] & 0xFF ];
			l ^= T2[ arr[40 + i] & 0xFF ];
			l ^= T3[ arr[32 + i] & 0xFF ];
			l ^= T4[ arr[24 + i] & 0xFF ];
		...
		for( int i = 0; i < 8; ++i )
		{
			int hi = (int)(longArr[i] >>> 32);
			int lo = (int)(longArr[i] & 0xFFFFFFFF);
			int offset = i << 3;
	...
	// -----------------------------------------------------------------------------------------------------------------
	private void G( byte[] N, byte[] data )
	{
		byte[] h = state;
		System.arraycopy( h, 0, savedState, 0, 64 );

		xor( h, N );
		F( h );

		System.arraycopy( h, 0, Ki, 0, 64 );
		xor( h, data );
		F( h );
		for( int i = 0; i < 11; ++i )
		{
	...
	// -----------------------------------------------------------------------------------------------------------------
	private static void add( byte[] A, int num )
	{
		int c = (A[ 63 ] & 0xFF) + (num & 0xFF);
		A[ 63 ] = (byte)c;
	...
	private final static byte[] Zero = new byte[64];

	private final static long[] T0 =
	{

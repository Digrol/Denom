...
import org.denom.crypt.hash.SHAKE;
...
public abstract class Ed448
{
	...
	private static final Binary DOM4_PREFIX = new Binary( "SigEd448".getBytes() );

	private static final int[] P = new int[] { 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFE, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF };
	private static final int[] L = new int[] { 0xAB5844F3, 0x2378C292, 0x8DC58F55, 0x216CC272, 0xAED63690, 0xC44EDB49, 0x7CCA23E9, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0xFFFFFFFF, 0x3FFFFFFF };
	...
	private static boolean decodePointVar( byte[] p, int pOff, boolean negate, PointExt r )
	{
		...
		if( !X448.sqrtRatioVar( u, v, r.x ) )
		{
			return false;
		}

		X448.normalize( r.x );
		if( x_0 == 1 && X448.isZeroVar( r.x ) )
		{
			return false;
		}
	...
	private static void dom4( SHAKE d, byte x, byte[] y )
	{
		Binary data = Bin( DOM4_PREFIX );
		data.add( x );
		data.add( y.length );
		data.add( y );
		d.process( data );
	...
	private static void encodePoint( PointExt p, byte[] r, int rOff )
	{
		...
		X448.encode( y, r, rOff );
		r[ rOff + POINT_BYTES - 1 ] = (byte)((x[ 0 ] & 1) << 7);
	}

	public static void generatePublicKey( byte[] sk, int skOff, byte[] pk, int pkOff )
	{
		SHAKE alg = new SHAKE( 256 );
		alg.process( new Binary( sk, skOff, SECRET_KEY_SIZE ) );
		Binary hash = alg.getHash( SCALAR_BYTES * 2 );

		byte[] s = new byte[ SCALAR_BYTES ];
		pruneScalar( hash.getBytes(), 0, s );
	...
	private static void implSignVar( SHAKE d, byte[] h, byte[] s, byte[] pk, int pkOff, byte[] ctx, byte[] m, int mOff, int mLen, byte[] sig, int sigOff )
	{
		byte phflag = 0x00;

		dom4( d, phflag, ctx );
		d.process( new Binary( h, SCALAR_BYTES, SCALAR_BYTES ) );
		d.process( new Binary( m, mOff, mLen ) );

		byte[] r = reduceScalar( d.getHash( h.length ).getBytes() );
		byte[] R = new byte[ POINT_BYTES ];
		scalarMultBaseEncodedVar( r, R, 0 );

		dom4( d, phflag, ctx );
		d.process( new Binary( R, 0, POINT_BYTES ) );
		d.process( new Binary( pk, pkOff, POINT_BYTES ) );
		d.process( new Binary( m, mOff, mLen ) );

		byte[] k = reduceScalar( d.getHash( h.length ).getBytes() );
	...
	private static PointExt pointCopy( PointExt p )
	{
		...
		X448.copy( p.x, 0, r.x, 0 );
		X448.copy( p.y, 0, r.y, 0 );
		X448.copy( p.z, 0, r.z, 0 );
		return r;
	}
	...
	{
		int off = block * PRECOMP_POINTS * 2 * X448.SIZE;

		for( int i = 0; i < PRECOMP_POINTS; ++i )
		{
			int mask = ((i ^ index) - 1) >> 31;
			Nat.cmov( X448.SIZE, mask, precompBase, off, p.x, 0 );
			off += X448.SIZE;
			Nat.cmov( X448.SIZE, mask, precompBase, off, p.y, 0 );
			off += X448.SIZE;
		}
	}
	...
	public synchronized static void precompute()
	{
		...
		X448.copy( B_x, 0, p.x, 0 );
		X448.copy( B_y, 0, p.y, 0 );
		pointExtendXY( p );

		precompBaseTable = pointPrecompVar( p, 1 << (WNAF_WIDTH_BASE - 2) );

		precompBase = new int[ PRECOMP_BLOCKS * PRECOMP_POINTS * 2 * X448.SIZE ];
		...
		for( int b = 0; b < PRECOMP_BLOCKS; ++b )
		{
			...
			for( int i = 0; i < PRECOMP_POINTS; ++i )
			{
				...
				X448.inv( q.z, q.z );
				X448.mul( q.x, q.z, q.x );
				X448.mul( q.y, q.z, q.y );

				X448.copy( q.x, 0, precompBase, off );
				off += X448.SIZE;
				X448.copy( q.y, 0, precompBase, off );
				off += X448.SIZE;
			}
		}
	}
	...
	public static void sign( byte[] sk, int skOff, byte[] ctx, byte[] m, int mOff, int mLen, byte[] sig, int sigOff )
	{
		...
		SHAKE d = new SHAKE( 256 );
		d.process( new Binary( sk, skOff, SECRET_KEY_SIZE ) );
		byte[] h = d.getHash( SCALAR_BYTES * 2 ).getBytes();
	...
	public static boolean verify( byte[] sig, int sigOff, byte[] pk, int pkOff, byte[] ctx, byte[] m, int mOff, int mLen )
	{
		...
		byte[] k = reduceScalar( d.getHash( SCALAR_BYTES * 2 ).getBytes() );
